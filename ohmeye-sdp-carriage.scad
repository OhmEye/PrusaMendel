...
include <ohmeye-sdp-holder.scad>

holder_separation=18;

fan_hole_separation=29; // check
fan_support_block=8;
fan_trap_width=3;
...
module ohmeye_fanmount() {

translate([-42,0,0])rotate([0,0,90])fan_mount();

}

module ohmeye_bits() {
	translate([14.4,23,0])rotate([0,0,45])cube([12,12,sdp_support_thickness*1.7]);
}
...
module gregs_x_carriage(with_fanmount=false) 
{
	difference()
	{
		...
		{
			if (with_fanmount) {
			translate([0,holder_separation/2+sdp_holder_length-fan_support_block,0])
			...
			for(i=[-1,1])
			{
				translate([50/2-sdp_holder_width/2,
					i*(sdp_holder_length+holder_separation)/2-
					sdp_holder_length/2,0])
				render()
				sdp_bearing_holder();
				...
				{
					translate([-50/2-sdp_holder_width/2,
						-sdp_holder_length-holder_separation/2,0])
					render()
					sdp_bearing_holder(-9.5);
				}
			}

			linear_extrude(height=sdp_support_thickness)
			translate([0,0,0])
			{
			barbell (
				[25,holder_separation/2],
				[25,-holder_separation/2],
				sdp_holder_width/2,sdp_holder_width/2,12,12);
		
//			square([40,50],true);				

			translate([-25,-holder_separation/2])
			circle(r=sdp_holder_width/2);

			barbell (
				[25-sdp_holder_width/2,
				-holder_separation/2-sdp_holder_length+sdp_holder_width/2],
				[-25+sdp_holder_width/2,
				-holder_separation/2-sdp_holder_length+sdp_holder_width/2],
				sdp_holder_width/2,sdp_holder_width/2,12,12);
			}

			translate([25+13.5+1,(sdp_holder_length*0.7+holder_separation/2)])
			...
			{
				rotate(45)
				assign(ehole=cos(45)*(50+sdp_holder_width+cos(45)*10),
					ehole_r=10,ehole_t=sdp_support_thickness*1.7)
				...
				for(j=[-1,1])
				if (!((i==-1) &&(j==1)))
				translate([23*i,18*j,(sdp_support_thickness*1.7+2)/2])
				cube([15,15,sdp_support_thickness*1.7+2],center=true);
			}
		}
		...
		// The big hole.
		rotate(45)
		translate([0,0,sdp_holder_height/2])
		assign(ehole=35,ehole_r=2)
		{
			cube([ehole-2*ehole_r,ehole,sdp_holder_height+20],center=true);
			cube([ehole,ehole-2*ehole_r,sdp_holder_height+20],center=true);


			translate([0,0,sdp_support_thickness+sdp_holder_height/2+0.55])cube([ehole-2*ehole_r+5,ehole+24.5,sdp_holder_height+10],center=true);
			translate([0,0,sdp_support_thickness+sdp_holder_height/2+0.55])cube([ehole+24.5,ehole-2*ehole_r+5,sdp_holder_height+10],center=true);


			for(i=[-1,1])
			for(j=[-1,1])
			translate([i*(ehole/2-ehole_r),j*(ehole/2-ehole_r),0])
			cylinder(r=ehole_r,h=sdp_holder_height+2,center=true,$fn=16);
		}
		...
		rotate(22)
		translate([0,25,-1])
		cylinder(r=m4_diameter/2,h=sdp_support_thickness*2+2,$fs=1);
...
//		for(i=[-1,1])
//		translate([38,i*18.4,-1])cube([10,5,15],true);
	}
}
...
module cable_tie_holes ()
{
	// Cable tie holes.
	for (i=[-1,1])
	translate([25-sdp_holder_width/2+1,(holder_separation/2+sdp_holder_length*0.7)*i,5])
	rotate([0,-30,0])
	{
	cube([2,4,20],center=true);
	translate([0,-sdp_holder_length*0.4*i,0])
	cube([2,4,20],center=true);
	}

	translate([-25+sdp_holder_width/2-1,-(holder_separation/2+sdp_holder_length*0.7),5])
	rotate([0,30,0])
	{
	cube([2,4,20],center=true);
	translate([0,sdp_holder_length*0.4,0])
	cube([2,4,20],center=true);
	}
}
...
module belt_clamp(version)
{
	difference()
	{
		...
		{
			translate([-belt_clamp_width-belt_width/2-0.5,sdp_holder_gap/2-0.25,-1])
...
module fan_mount() 
{
	difference()
	{
		translate([0,0.15,1.1])rotate([-15,0,0])union ()
		{
...
//		translate([fan_support_block/2+25+1-sdp_holder_width/2,0,fan_support_block/2])
//		cube([fan_support_block,fan_support_block+2,fan_support_block+2],true);
		rotate([-15,0,0])fan_mount_holes();
	}
		difference(){
			translate([-18.5,-34,0])
			cube([fan_hole_separation+fan_support_block,fan_support_thickness+30,lm8uu_support_thickness]);
			rotate([-15,0,0])fan_mount_holes();
		}
}
...
module fan_mount_holes()
{
	for(i=[-1,1])
	{
		translate([i*fan_hole_separation/2,0,1+fan_hole_height])
		{
			rotate([90,0,0])
			rotate(180/8)
	...
	intersection()
	{
		translate([0,0,1.5+fan_hole_separation/2+fan_hole_height])
		rotate([-90,0,0])
		cylinder(r=fan_diameter/2+1.9,h=fan_support_thickness+2,center=true);
